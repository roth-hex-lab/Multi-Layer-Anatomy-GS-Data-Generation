...
if __name__ == "__main__":
    ...
    # settings
    OUT_PATH = os.path.join(ROOT_DIR, 'generated/fullbody/organs')
    ...
    TRANSFER_FUNC = "data/lut_fullbody/organs.txt"
    OUT_FORMAT = ".txt"
    N_VIEWS = 512
    MAX_VARIABILITY = 1.0
    ...
    print(f"Rendering {VOLUME} to {OUT_PATH}")
    ...
    with open(os.path.join(OUT_PATH, 'settings.json'), 'w', encoding='utf-8') as file:
        data = dict(VOLUME=VOLUME, ENVMAP=ENVMAP, SAMPLES=SAMPLES,TRANSFER_FUNC=TRANSFER_FUNC, N_VIEWS=N_VIEWS, WINDOW_LEFT=WINDOW_LEFT, WINDOW_WIDTH=WINDOW_WIDTH, CUTOFF=CUTOFF, ENV_STRENGTH=ENV_STRENGTH, CLIP_MIN=str(CLIP_MIN))
    ...
    for i in range(N_VIEWS):
        ...
        cam_target = center + (sample_inside_unit_sphere(rng.standard_normal(3)) * radius * 0.8 * variability)                      # Cam target somewhere withoing 80% radius of volume, increasing variability
        ...
        cam_pos = cam_pos_moved_out + (sample_inside_unit_sphere(rng.standard_normal(3)) * radius * 0.4 * variability)              # randomize camera position by moving up to 0.4 times radius around
    ...
    renderer.to_pointcloud(1.8, pc_path)
