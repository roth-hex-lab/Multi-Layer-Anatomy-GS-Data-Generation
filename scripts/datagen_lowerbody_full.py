...
if __name__ == "__main__":
    ...
    # settings
    OUT_PATH = os.path.join(ROOT_DIR, 'generated/lowerbody/full')
    VOLUME = os.path.join(ROOT_DIR, 'data/Lower Body/3.000000-ARTERIAL-34922/1-0001.dcm')
    ENVMAP = os.path.join(ROOT_DIR, 'data/table_mountain_2_puresky_1k.hdr')
    TRANSFER_FUNC = "data/lut_leg/full_gen_two.txt"
    OUT_FORMAT = ".txt"
    N_VIEWS = 512
    MAX_VARIABILITY = 1.0
    ...
    SCALE_ADJUST_Y = 0.001
    SCALE_ADJUST_Z = 0.0006
    ...
    renderer.vol_clip_max = CLIP_MAX
    ...
    with open(os.path.join(OUT_PATH, 'settings.json'), 'w', encoding='utf-8') as file:
        data = dict(VOLUME=VOLUME, ENVMAP=ENVMAP, SAMPLES=SAMPLES,TRANSFER_FUNC=TRANSFER_FUNC, N_VIEWS=N_VIEWS, WINDOW_LEFT=WINDOW_LEFT, WINDOW_WIDTH=WINDOW_WIDTH, CUTOFF=CUTOFF, DENSITY_SCALE=DENSITY_SCALE, ENV_STRENGTH=ENV_STRENGTH, CLIP_MIN=str(CLIP_MIN), CLIP_MAX=str(CLIP_MAX))
    ...
    renderer.to_pointcloud(6, pc_path)
